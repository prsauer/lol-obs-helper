...
export const MatchDetails = ({ matchId }: { matchId: string }) => {
  ...
  const partIndex =
    gameTimeline.info.participants.findIndex((p) => p.puuid === selectedChamp) +
    1;
  ...
  const selectedInfo = gameInfoQuery.data?.data?.info.participants.find(
        ...
        {gameInfoQuery.data?.data?.info.participants.map((p) => (
          <div
            ...
            className={
              ...
              (selectedChamp === p.puuid ? "border-green-100 border-2" : "")
            }
            onClick={() => {
          ...
          >
            ...
            <div
              className={
                "text-sm " +
                (p.teamId === 100 ? "text-green-400" : "text-red-400")
              }
            >
              {p.summonerName}
            </div>
