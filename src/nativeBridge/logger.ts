...
function getAppDataPath() {
  switch (process.platform) {
    ...
    case 'darwin': {
      return path.join(process.env.HOME as string, 'Library', 'Application Support', 'lol-obs-helper');
    }
    case 'win32': {
      return path.join(process.env.APPDATA as string, 'lol-obs-helper');
    }
    case 'linux': {
      return path.join(process.env.HOME as string, '.lol-obs-helper');
    }
    default: {
